...
func TestCreateAlterRoleQuery(t *testing.T) {
	...
	for _, c := range cases {
		out := createAlterRoleQuery(schema.TestResourceDataRaw(t, resourcePostgreSQLRoleParameter().Schema, c.resource))
		if out != c.expected {
...
func TestResetRoleQuery(t *testing.T) {
	...
	for _, c := range cases {
		out := createResetAlterRoleQuery(schema.TestResourceDataRaw(t, resourcePostgreSQLRoleParameter().Schema, c.resource))
		if out != expected {
